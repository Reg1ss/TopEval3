...
void testDetectionModules(std::string inputPath, std::string outputPath, std::string id,
                          std::shared_ptr<ObjectDetection> AlgoDet, int model, std::string modelPath);
//get Xml Result
void write2Xml(std::string outputPath, std::string id, std::vector<ObjectDetectionResult> res_v);
//get Xml Result with confidence score
...
int main(int argc, char *argv[]) {
    ...
    int model = 2;  //model=1:xml result||model=2:xml result with score

    //test one root folder
    std::string inputPathO = "/home/kuangrx/framesTunnel"; //Root input folder Path
    std::string outputPathO = "/home/kuangrx/xmlsf/v0.6_tunnel";//Root output folder Path
    int folderCount = 1;//Folders number
        ...
        for (int j = 1; j < totalFiles + 1; j++) {
            if (j < 10) {
                ...
                if (access(path.data(), F_OK) == 0) {
                    ...
                    testDetectionModules(inputPath, outputPath, id, AlgoDet, model ,modelPath);
                }
            }
            if (9 < j < 100) {
